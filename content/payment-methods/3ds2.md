...
When you collect cardholder data yourself, you also need to collect the contextual information from the customer's device (fingerprint). This is only relevant when creating `direct` card orders. **Not** when using [Payment Components](/docs/payment-components) or [Payment pages](/docs/payment-pages), where MultiSafepay collects the contextual information on your behalf.
